...
@objc public class RNJWPlayerViewController : JWPlayerViewController, JWPlayerViewControllerDelegate, JWDRMContentKeyDataSource {

    @objc public var parentView:RNJWPlayerView!
...
//        self.player.avDelegate = nil
    ...
    public override func jwplayerIsReady(_ player:JWPlayer) {
    ...
    public override func jwplayer(_ player:JWPlayer, failedWithError code:UInt, message:String) {
        super.jwplayer(player, failedWithError:code, message:message)
        parentView?.onPlayerError?(["error": message])
        parentView?.playerFailed = true
    }

    public override func jwplayer(_ player:JWPlayer, failedWithSetupError code:UInt, message:String) {
        super.jwplayer(player, failedWithSetupError:code, message:message)
        parentView?.onSetupPlayerError?(["error": message])
        parentView?.playerFailed = true
    }

    public override func jwplayer(_ player:JWPlayer, encounteredWarning code:UInt, message:String) {
        super.jwplayer(player, encounteredWarning:code, message:message)
        parentView?.onPlayerWarning?(["warning": message])
    }

    public override func jwplayer(_ player:JWPlayer, encounteredAdError code:UInt, message:String) {
        super.jwplayer(player, encounteredAdError:code, message:message)
        parentView?.onPlayerAdError?(["error": message])
    }


    public override func jwplayer(_ player:JWPlayer, encounteredAdWarning code:UInt, message:String) {
    ...
    public override func playerView(_ view:JWPlayerView, sizeChangedFrom oldSize:CGSize, to newSize:CGSize) {
    ...
    public override func onAdTimeEvent(_ time:JWTimeData) {
        super.onAdTimeEvent(time)
        parentView?.onAdTime?(["position": time.position, "duration": time.duration])
    }

    public override func onMediaTimeEvent(_ time:JWTimeData) {
...
//        }
    ...
    public override func pictureInPictureControllerDidStopPictureInPicture(_ pictureInPictureController:AVPictureInPictureController) {
        super.pictureInPictureControllerDidStopPictureInPicture(pictureInPictureController)
    }

    public override func pictureInPictureControllerDidStartPictureInPicture(_ pictureInPictureController:AVPictureInPictureController) {
        super.pictureInPictureControllerDidStartPictureInPicture(pictureInPictureController)
    }

    public override func pictureInPictureControllerWillStopPictureInPicture(_ pictureInPictureController:AVPictureInPictureController) {
        super.pictureInPictureControllerWillStopPictureInPicture(pictureInPictureController)
    }

    public override func pictureInPictureController(_ pictureInPictureController: AVPictureInPictureController, failedToStartPictureInPictureWithError error: Error) {
        super.pictureInPictureController(pictureInPictureController, failedToStartPictureInPictureWithError: error)
    }

    public override func pictureInPictureControllerWillStartPictureInPicture(_ pictureInPictureController:AVPictureInPictureController) {
        super.pictureInPictureControllerWillStartPictureInPicture(pictureInPictureController)
    }

    public override func pictureInPictureController(_ pictureInPictureController:AVPictureInPictureController, restoreUserInterfaceForPictureInPictureStopWithCompletionHandler completionHandler:@escaping (Bool) -> Void) {
    ...
    public override func jwplayerContentIsBuffering(_ player:JWPlayer) {
        super.jwplayerContentIsBuffering(player)
        parentView?.onBuffer?([:])
    }

    public override func jwplayer(_ player:JWPlayer, isBufferingWithReason reason:JWBufferReason) {
        super.jwplayer(player, isBufferingWithReason:reason)
        parentView?.onBuffer?([:])
    }

    public override func jwplayer(_ player:JWPlayer, updatedBuffer percent:Double, position time:JWTimeData) {
        super.jwplayer(player, updatedBuffer:percent, position:time)
        parentView?.onUpdateBuffer?(["percent": percent, "position": time as Any])
    }

    public override func jwplayer(_ player:JWPlayer, didFinishLoadingWithTime loadTime:TimeInterval) {
        super.jwplayer(player, didFinishLoadingWithTime:loadTime)
        parentView?.onLoaded?([:])
    }

    public override func jwplayer(_ player:JWPlayer, isAttemptingToPlay playlistItem:JWPlayerItem, reason:JWPlayReason) {
        super.jwplayer(player, isAttemptingToPlay:playlistItem, reason:reason)
        parentView?.onAttemptPlay?([:])
    }

    public override func jwplayer(_ player:JWPlayer, isPlayingWithReason reason:JWPlayReason) {
    ...
    public override func jwplayer(_ player:JWPlayer, willPlayWithReason reason:JWPlayReason) {
        super.jwplayer(player, willPlayWithReason:reason)
        parentView?.onBeforePlay?([:])
    }

    public override func jwplayer(_ player:JWPlayer, didPauseWithReason reason:JWPauseReason) {
    ...
    public override func jwplayer(_ player:JWPlayer, didBecomeIdleWithReason reason:JWIdleReason) {
        super.jwplayer(player, didBecomeIdleWithReason:reason)
        parentView?.onIdle?([:])
    }

    public override func jwplayer(_ player:JWPlayer, isVisible:Bool) {
        super.jwplayer(player, isVisible:isVisible)
        parentView?.onVisible?(["visible": isVisible])
    }

    public override func jwplayerContentWillComplete(_ player:JWPlayer) {
        super.jwplayerContentWillComplete(player)
        parentView?.onBeforeComplete?([:])
    }

    public override func jwplayerContentDidComplete(_ player:JWPlayer) {
        super.jwplayerContentDidComplete(player)
        parentView?.onComplete?([:])
    }

    public override func jwplayer(_ player:JWPlayer, didLoadPlaylistItem item:JWPlayerItem, at index:UInt) {
...
//        item.addObserver(self, forKeyPath:"playbackLikelyToKeepUp", options:.new, context:nil)
    ...
    public override func jwplayer(_ player:JWPlayer, didLoadPlaylist playlist:[JWPlayerItem]) {
...
//            ]
    ...
    public override func jwplayerPlaylistHasCompleted(_ player:JWPlayer) {
        super.jwplayerPlaylistHasCompleted(player)
        parentView?.onPlaylistComplete?([:])
    }

    public override func jwplayer(_ player:JWPlayer, usesMediaType type:JWMediaType) {
        super.jwplayer(player, usesMediaType:type)
    }

    public override func jwplayer(_ player:JWPlayer, seekedFrom oldPosition:TimeInterval, to newPosition:TimeInterval) {
        super.jwplayer(player, seekedFrom:oldPosition, to:newPosition)
        parentView?.onSeek?(["from": oldPosition, "to": newPosition])
    }

    public override func jwplayerHasSeeked(_ player:JWPlayer) {
        super.jwplayerHasSeeked(player)
        parentView?.onSeeked?([:])
    }

    public override func jwplayer(_ player:JWPlayer, playbackRateChangedTo rate:Double, at time:TimeInterval) {
        super.jwplayer(player, playbackRateChangedTo:rate, at:time)
        parentView?.onRateChanged?(["rate": rate, "at": time])
    }

    public override func jwplayer(_ player:JWPlayer, updatedCues cues:[JWCue]) {
        super.jwplayer(player, updatedCues:cues)
    }

    // MARK: - JWPlayer Ad Delegate

    public override func jwplayer(_ player: JWPlayer, adEvent event: JWAdEvent) {
    ...
    public override func jwplayer(_ player:JWPlayer, audioTracksUpdated levels:[JWMediaSelectionOption]) {
        super.jwplayer(player, audioTracksUpdated:levels)
        parentView?.onAudioTracks?([:])
    }

    public override func jwplayer(_ player:JWPlayer, audioTrackChanged currentLevel:Int) {
        super.jwplayer(player, audioTrackChanged:currentLevel)
    }

    public override func jwplayer(_ player:JWPlayer, captionPresented caption:[String], at time:JWTimeData) {
        super.jwplayer(player, captionPresented:caption, at:time)
    }

    public override func jwplayer(_ player:JWPlayer, captionTrackChanged index:Int) {
        super.jwplayer(player, captionTrackChanged:index)
    }

    public override func jwplayer(_ player:JWPlayer, qualityLevelChanged currentLevel:Int) {
        super.jwplayer(player, qualityLevelChanged:currentLevel)
    }

    public override func jwplayer(_ player:JWPlayer, qualityLevelsUpdated levels:[JWVideoSource]) {
        super.jwplayer(player, qualityLevelsUpdated:levels)
    }

    public override func jwplayer(_ player:JWPlayer, updatedCaptionList options:[JWMediaSelectionOption]) {
